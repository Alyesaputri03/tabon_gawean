...
$tabel_supabase = 'layanan_ipds';
$nama_divisi    = 'Tim IPDS & Jaringan';
$ikon_default   = 'network';
$pesan_sukses   = '';
$pesan_error    = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['tambah_layanan'])) {
    ...
    if (!empty($nama_layanan) && !empty($url_link)) {
        ...
        if ($res['status'] >= 200 && $res['status'] < 300) {
            $pesan_sukses = "Layanan Tim IPDS & Jaringan baru berhasil ditambahkan!";
        } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_layanan'])) {
    ...
    if ($id > 0 && !empty($nama_layanan) && !empty($url_link)) {
        ...
        if ($res['status'] >= 200 && $res['status'] < 300) {
            $pesan_sukses = "Perubahan layanan Tim IPDS berhasil disimpan!";
        } else {
...
// Ambil data aktif dari Supabase (dengan proteksi array)
...
// Ambil data sampah khusus divisi IPDS
...
<body class="bg-slate-50 text-slate-800 antialiased min-h-screen flex flex-col justify-between">
 ...
 <!-- Header Navigasi -->
  <header class="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-sm">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
      ...
      <div class="flex items-center gap-3">
        <a 
          href="index.php" 
          class="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs sm:text-sm font-bold text-slate-700 bg-slate-100 hover:bg-cyan-50 hover:text-cyan-700 transition border border-slate-200 shadow-sm"
        >
        ...
        <button 
          ...
          title="Buka menu pemulihan layanan IPDS terhapus"
  ...
  <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full flex-1">
    ...
    <!-- Banner Tim IPDS & Jaringan -->
    <div class="bg-white rounded-2xl border border-slate-200 p-6 sm:p-8 shadow-sm mb-8">
      <div class="flex items-start gap-4">
        <div class="w-14 h-14 rounded-2xl bg-cyan-50 text-cyan-600 flex items-center justify-center shrink-0">
          <i data-lucide="network" class="w-7 h-7"></i>
        </div>
        <div>
          <span class="text-xs font-bold uppercase tracking-wider text-cyan-600">Infrastruktur Teknologi Informasi & Jaringan Satker</span>
          ...
          <p class="text-sm text-slate-600 mt-2 max-w-3xl">
            Pusat monitoring infrastruktur server satker, portal VPN dinas, manajemen bandwidth & jaringan internet, perbaikan perangkat keras (hardware), repositori aplikasi internal, dan helpdesk TI BPS Kota Yogyakarta.
          </p>
        </div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <?php foreach ($data_aktif as $item): ?>
        ...
        ?>
        <div class="bg-white rounded-2xl border border-slate-200/80 p-6 flex flex-col justify-between hover:border-cyan-400 hover:shadow-md transition relative group">
          <div>
            ...
            <div class="flex items-center justify-between gap-2 mb-4">
              ...
              <div class="flex items-center gap-1">
                ...
                <button 
                  ...
                  title="Edit Layanan"
                  class="w-7 h-7 rounded-lg text-slate-400 hover:text-cyan-600 hover:bg-cyan-50 flex items-center justify-center transition"
                >
                  <i data-lucide="pencil" class="w-3.5 h-3.5"></i>
                </button>

                <!-- Tombol Hapus (Soft Delete) -->
                <form method="POST" action="" onsubmit="return confirm('Pindahkan layanan IPDS ini ke Pemulihan?');" class="inline">
          ...
          <div class="mt-8 pt-4 border-t border-slate-100 flex items-center justify-between">
            <a href="<?= htmlspecialchars($link_tujuan); ?>" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1.5 text-xs font-semibold text-cyan-600 hover:text-cyan-700 transition">
      ...
      <button 
        ...
        class="group min-h-[220px] rounded-2xl border-2 border-dashed border-slate-300 hover:border-cyan-500 hover:bg-cyan-50/50 p-6 flex flex-col items-center justify-center text-center transition"
      >
        <div class="w-12 h-12 rounded-full bg-cyan-100 text-cyan-600 group-hover:bg-cyan-600 group-hover:text-white flex items-center justify-center transition shadow-sm mb-3">
          <i data-lucide="plus" class="w-6 h-6"></i>
        </div>
        <span class="font-bold text-slate-700 group-hover:text-cyan-600 text-sm sm:text-base transition">
          Tambah Layanan Baru
        </span>
        <p class="text-xs text-slate-400 mt-1">Tambahkan tautan sistem IT, monitoring server, atau helpdesk teknis</p>
      </button>
    </div>
  </main>
    ...
    <div class="bg-white rounded-3xl border border-slate-200 shadow-2xl max-w-lg w-full p-6 sm:p-8">
      ...
      <form method="POST" action="" class="space-y-4">
        <div>
          ...
          <input type="text" name="nama_layanan" required placeholder="Contoh: Portal Monitoring Jaringan Satker" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 outline-none transition">
        </div>
        <div>
          <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Kategori / Badge</label>
          <input type="text" name="kategori" placeholder="Contoh: Jaringan / Server / Helpdesk / VPN" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 outline-none transition">
        </div>
        <div>
          <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Tautan URL *</label>
          <input type="url" name="url_link" required placeholder="https://..." class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 outline-none transition">
        </div>
        <div>
          <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Deskripsi Singkat</label>
          <textarea name="deskripsi" rows="2" placeholder="Jelaskan ringkasan fungsi layanan infrastruktur TI ini..." class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 outline-none transition"></textarea>
        </div>
        ...
        <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
          ...
          <button type="submit" name="tambah_layanan" class="px-5 py-2.5 rounded-xl text-xs font-bold text-white bg-cyan-600 hover:bg-cyan-700 shadow-md shadow-cyan-500/20 transition flex items-center gap-1.5">
    ...
    <div class="bg-white rounded-3xl border border-slate-200 shadow-2xl max-w-lg w-full p-6 sm:p-8">
      <div class="flex items-center justify-between pb-4 border-b border-slate-100 mb-5">
        <div class="flex items-center gap-2">
          ...
          <h3 class="font-bold text-slate-900 text-base sm:text-lg">Edit Layanan IPDS</h3>
        </div>
      ...
      <form method="POST" action="" class="space-y-4">
        ...
        <div>
          <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Nama Layanan *</label>
          <input type="text" name="nama_layanan" id="edit_nama" required class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 outline-none transition">
        </div>
        <div>
          <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Kategori / Badge</label>
          <input type="text" name="kategori" id="edit_kategori" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 outline-none transition">
        </div>
        <div>
          <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Tautan URL *</label>
          <input type="url" name="url_link" id="edit_url" required class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 outline-none transition">
        </div>
        <div>
          <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Deskripsi Singkat</label>
          <textarea name="deskripsi" id="edit_deskripsi" rows="2" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 outline-none transition"></textarea>
        </div>
        ...
        <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
          ...
          <button type="submit" name="edit_layanan" class="px-5 py-2.5 rounded-xl text-xs font-bold text-white bg-cyan-600 hover:bg-cyan-700 shadow-md shadow-cyan-500/20 transition flex items-center gap-1.5">
  ...
  <!-- MODAL 3: PEMULIHAN KHUSUS TIM IPDS -->
    ...
    <div class="bg-white rounded-3xl border border-slate-200 shadow-2xl max-w-2xl w-full p-6 sm:p-8 flex flex-col max-h-[85vh]">
      ...
      <div class="flex items-center justify-between pb-4 border-b border-slate-100 mb-4">
        <div class="flex items-center gap-2">
          ...
          <div>
            <h3 class="font-bold text-slate-900 text-base sm:text-lg">Pemulihan Layanan Tim IPDS</h3>
